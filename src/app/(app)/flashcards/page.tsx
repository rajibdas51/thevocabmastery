...
export default function FlashcardsPage() {
  ...
  useEffect(() => {
    ...
    getCategories(profile.id).then(({ data }) => {
      setCategories(data ?? [])
      // Default stays "All Categories" (catId stays '')
    })
  }, [profile])
  ...
  const start = async () => {
    ...
    // Fetch a large pool (category filter applied server-side,
    // prefix + learned-only applied client-side below)
    ...
    if (prefixEnabled && prefix.trim()) {
      ...
      pool = pool.filter(w => w.word.toLowerCase().startsWith(p))
    }

    if (learnedOnly) {
      pool = pool.filter(w => w.user_progress?.status === 'learned')
    }

    if (!pool.length) {
      toast(
        learnedOnly
          ? "No learned words match these filters. Mark some words as learned first!"
    ...
    const finalWords = cardCount === 'all'
      ? shuffledPool
      : shuffledPool.slice(0, parseInt(cardCount))
        ...
        <Card className="p-5 sm:p-6 space-y-5">
          ...
          <Select
            ...
            options={[
              ...
              ...categories.map(c => ({ value: c.id, label: `${c.name} (${c.word_count} words)` })),
            ]}
          />
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs mt-0.5" style={{ color: 'var(--text3)' }}>
                  Review only words you've marked as learned
                </p>
              </div>
            </div>
            <button
            ...
            {prefixEnabled && (
              <div className="space-y-3">
                ...
                <p className="text-[11px]" style={{ color: 'var(--text3)' }}>
                  Deck will only include words starting with{' '}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {COUNT_OPTIONS.map(o => (
                <button
                  key={o.value} onClick={() => setCardCount(o.value)}
                  className="px-4 py-2 rounded-lg text-sm font-semibold border transition-all"
                  style={{
                    background:  cardCount === o.value ? 'var(--accent)' : 'var(--bg3)',
                    borderColor: cardCount === o.value ? 'var(--accent)' : 'var(--border2)',
                    color:       cardCount === o.value ? '#fff'          : 'var(--text2)',
                  }}
                >
                  {o.label}
                </button>
      ...
      <div className="flex items-start justify-between px-4 sm:px-8 pt-6 sm:pt-8 pb-0">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: 'var(--text2)' }}>
            {catId ? categories.find(c => c.id === catId)?.name : 'All Categories'}
            {learnedOnly && ' · Learned Only'}
            {prefixEnabled && prefix && ` · "${prefix.toUpperCase()}"`}
          </p>
        </div>
